...
fn main() {
    ...
    let requester =
        Requester::<AddRequest, AddReply>::new(&participant, "CalcService", None, None).unwrap();

    let replier =
        Replier::<AddRequest, AddReply>::new(&participant, "CalcService", None, None).unwrap();
